    ...
    <div class="form-group">
        ...
        <div data-image-container="image" class="<?= !empty($row->settings->image) ? null : 'd-none' ?>">
            ...
            <div class="custom-control custom-checkbox my-2">
                <input id="<?= $row->biolink_block_id . '_image_remove' ?>" name="image_remove" type="checkbox" class="custom-control-input" onchange="this.checked ? document.querySelector('#<?= 'pdf_document_image_' . $row->biolink_block_id ?>').classList.add('d-none') : document.querySelector('#<?= 'pdf_document_image_' . $row->biolink_block_id ?>').classList.remove('d-none')">
                <label class="custom-control-label" for="<?= $row->biolink_block_id . '_image_remove' ?>">
                    <span class="text-muted"><?= l('global.delete_file') ?></span>
                </label>
            </div>
        </div>
        <input id="<?= 'pdf_document_image_' . $row->biolink_block_id ?>" type="file" name="image" accept="<?= \Altum\Uploads::array_to_list_format($data->biolink_blocks['pdf_document']['whitelisted_thumbnail_image_extensions']) ?>" class="form-control-file altum-file-input" />
