                                        ...
                                        <?php foreach(['scroll', 'fixed'] as $background_attachment): ?>
                                            <div class="col-6">
                                                <label class="btn btn-light btn-block <?= $data->link->settings->background_attachment == $background_attachment ? 'active"' : null?>">
                                                    <input type="radio" name="background_attachment" value="<?= $background_attachment ?>" class="custom-control-input" <?= ($data->values['settings']['background_attachment'] ?? null) == $background_attachment ? 'checked="checked"' : null?> />
                                    ...
                                    <div class="<?= $data->link->is_verified ? null : 'container-disabled' ?>">
                                        <div class="form-group">
                                            ...
                                            <select id="verified_location" name="verified_location" class="custom-select">
                                                <option value="" <?= $data->link->settings->verified_location == '' ? 'selected="selected"' : null?>><?= l('global.none') ?></option>
        ...
        let block_expanded_content_init = () => {
            ...
            switch (link_type) {
                ...
                case 'heading':
                    ...
                    $(update_form_content.querySelector('select[name="heading_type"]')).off().on('change', event => {
                        let heading_type = $(event.currentTarget).find(':selected').val();
                        biolink_link.find('[data-text]').removeClass('h1 h2 h3 h4 h5 h6').addClass(heading_type);
                    });
