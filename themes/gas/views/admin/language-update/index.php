...
<?php if($data->type): ?>
    ...
    <div class="alert <?= $total > ini_get('max_input_vars') ? 'alert-danger' : 'alert-info' ?>" role="alert">
        <?= sprintf(l('admin_languages.info_message.max_input_vars'), nr(ini_get('max_input_vars'))) ?>
    </div>
<?php endif ?>
                            ...
                            <div class="row">
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <select id="<?= $form_key ?>" name="<?= $form_key ?>" class="custom-select <?= \Altum\Alerts::has_field_errors($form_key) ? 'is-invalid' : null ?> <?= !isset(\Altum\Language::get($data->language['name'])[$key]) || (isset(\Altum\Language::get($data->language['name'])[$key]) && empty(\Altum\Language::get($data->language['name'])[$key])) ? 'border-info' : null ?>" <?= $index++ >= ini_get('max_input_vars') ? 'readonly="readonly"' : null ?>>
                        ...
                        <?php else: ?>
                            <div class="row" data-display-container>
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="<?= $form_key ?>">&nbsp;</label>
                                        <textarea data-display-input id="<?= $form_key ?>" name="<?= $form_key ?>" class="form-control <?= \Altum\Alerts::has_field_errors($form_key) ? 'is-invalid' : null ?> <?= !isset(\Altum\Language::get($data->language['name'])[$key]) || (isset(\Altum\Language::get($data->language['name'])[$key]) && empty(\Altum\Language::get($data->language['name'])[$key])) ? 'border-info' : null ?>" <?= $index++ >= ini_get('max_input_vars') ? 'readonly="readonly" data-toggle="tooltip" data-html="true" title="' . (str_replace('"', '\'', sprintf(l('admin_languages.info_message.max_input_vars'), ini_get('max_input_vars')))) . '"' : null ?>><?= \Altum\Language::get($data->language['name'])[$key] ?? null ?></textarea>
            ...
            const response = await fetch(openai_api_endpoint, {
                ...
                body: JSON.stringify({
                    ...
                    'messages': [
                        {
                            'role': 'system',
                            'content': `You are a professional translator that can translate strings from English to ${language_to_translate_to}. You should exclude PHP sprintf type of variables and leave them as they are.`
                        },
                        {
                            'role': 'user',
