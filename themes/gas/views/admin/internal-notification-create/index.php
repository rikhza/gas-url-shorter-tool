        ...
        <form id="form" action="" method="post" role="form">
            ...
            <div class="form-group" data-character-counter="textarea">
                <label for="description" class="d-flex justify-content-between align-items-center">
                    <span><i class="fas fa-fw fa-sm fa-paragraph text-muted mr-1"></i> <?= l('admin_internal_notifications.main.description') ?></span>
            ...
            <div class="form-group">
                ...
                <input type="url" id="url" name="url" value="<?= $data->values['url'] ?>" class="form-control <?= \Altum\Alerts::has_field_errors('url') ? 'is-invalid' : null ?>" maxlength="512" />
            ...
            <div class="form-group" data-segment="filter">
                <label for="source"><i class="fas fa-fw fa-sm fa-toggle-on text-muted mr-1"></i> <?= l('admin_users.main.source') ?></label>
                <div class="row">
                    <?php foreach(['direct', 'admin_create', 'admin_api_create', 'facebook', 'twitter', 'discord', 'google', 'linkedin'] as $source): ?>
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-switch">
...
<script>
    ...
    document.querySelectorAll('#filters_is_newsletter_subscribed,[name="filters_plans[]"],[name="filters_status[]"],[name="filters_countries[]"],[name="filters_continents[]"],[name="filters_source[]"]').forEach(element => element.addEventListener('change', async event => {
        await get_segment_count();
    }));
