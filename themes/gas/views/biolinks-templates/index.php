            ...
            <?php foreach($data->biolinks_templates as $row): ?>
                <div class="col-lg-6 col-xl-4">
                    <div class="custom-row mb-4 d-flex flex-column justify-content-between">
                        ...
                        <a href="<?= $row->url ?>" target="_blank" class="btn btn-block btn-sm btn-light"><i class="fas fa-fw fa-sm fa-external-link-alt mr-1"></i> <?= l('biolinks_templates.preview') ?></a>
                        <button type="button" class="btn btn-block btn-sm btn-primary" data-toggle="modal" data-target="#create_biolink" onclick="document.querySelector(`input[name='biolink_template_id']`).value = <?= $row->biolink_template_id ?>;"><i class="fas fa-fw fa-sm fa-plus-circle mr-1"></i> <?= l('biolinks_templates.choose') ?></button>
                    </div>
                </div>
            <?php endforeach ?>
        </div>
