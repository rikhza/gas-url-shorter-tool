    ...
    public function __construct() {
        ...
        if(\Altum\Authentication::check()) {
            ...
            if(!$user->plan || ($user->plan && ((new \DateTime()) > (new \DateTime($user->plan_expiration_date)) && $user->plan_id != 'free') || !$user->plan->status)) {
                ...
                if(!in_array(\Altum\Router::$controller_key, ['index', 'contact', 'page', 'pages', 'account', 'account-logs', 'account-delete', 'account-api', 'logout', 'register', 'teams-system', 'teams-member', 'teams-members']) && \Altum\Router::$path != 'admin') {
                    redirect('plan/new');
                }
            }
